...
#define BARRA_IZQUIERDA 3
#define BARRA_DERECHA 105
...
void Administrador::SetWindowSize(int width, int height) {
    HWND hwnd = GetConsoleWindow();  // Obtener el identificador de la ventana de la consola
    if (hwnd != NULL) {
        RECT rect;
        GetWindowRect(hwnd, &rect);  // Obtener las dimensiones actuales de la ventana
        int posX = rect.left;
        int posY = rect.top;
        
        // Ajustar el tamaño de la ventana
        SetWindowPos(hwnd, NULL, posX, posY, width, height, SWP_SHOWWINDOW);
    }
}
 ...
 ___/\\\\\\\\\\\\\____/\\\/////////\\\_____/\\\////////__\/////\\\///__\/////\\\///__       
  __/\\\/////////\\\__\//\\\______\///____/\\\/_______________\/\\\_________\/\\\_____      
   _\/\\\_______\/\\\___\////\\\__________/\\\_________________\/\\\_________\/\\\_____     
    ...
    gotoxy(BARRA_IZQUIERDA, 12);
    std::cout << AB << "[";
    gotoxy(BARRA_DERECHA, 12);
    std::cout << AB << "]";
    for (int i = BARRA_IZQUIERDA + 1; i < 95; i++) {
        gotoxy(i, 12);
        std::cout << VB << BLOCK; std::cout<<RTNC;
        Sleep(70);
    }
    gotoxy(40, 14);
    std::cout << VB << "PROGRAMA CARGADO..." << RTNC;
...
void Administrador::ingresar() {
    ...
    do {
        ...
        std::cout << RF << "TaskMaster v0.1" <<RTNC;
        gotoxy(5, 5);
        std::cout << AQF << "Quieres agregar una nueva tarea?" <<RTNC;
        gotoxy(5, 7);
...
void Administrador::eliminar () {
        ...
        while (auxString != "S" && !encontrado) {
            ...
            gotoxy(3, x + 5);
            if (auxString != "S" && !encontrado) {
                gotoxy(3, x + 7);
                std::cout << RF << "Respuesta incorrecta, ingresa un nombre valido..." <<RTNC; pausa();
            } else if (auxString != "S" && encontrado){
                gotoxy(3, x + 7);
                listaTareas.eliminar(listaTareas.localizaNombre(auxString));
                std::cout << RF << "Tarea borrada con exito..." <<RTNC; pausa();
            }
        }
}
